...
- **Enrich** — AI photo classification into a controlled taxonomy (`/enrich.html`). Works with operator-hosted Ollama or LM Studio, or through the cloud with OpenAI, OpenRouter, Venice, or Ollama's cloud models.
...
| `OPENAI_API_KEY` / `LMSTUDIO_*` / `OPENROUTER_*` / `OLLAMA_*` / `VENICE_*` | for Enrich | Provider connections and vision-model identifiers; configure the same values under Settings → AI Providers |
