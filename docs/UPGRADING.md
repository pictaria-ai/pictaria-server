...
persistent-state contract to **20**. Startup creates the standard complete
pre-migration recovery snapshot before adding the completed stack-check cache and failed-pass retry checkpoints.
...
backlog. Completed checks and failed-pass retry delays survive restarts. Missing
Immich embeddings are reported as incomplete checks and retried with backoff;
other groups can still progress. These reads do not invoke an AI
