...
test('a complete legacy installation upgrades, restarts, backs up, and restores without data loss', async () => {
  ...
  try {
    ...
    const aiTicket = first.enrichment.curate.aiLimits.start({ role: 'stack', inputKey: 'd'.repeat(64),
      backendKey: 'e'.repeat(64), photoIds: ['timing-fixture'] }, first.enrichment.curate.aiAttempts);
    first.enrichment.curate.aiLimits.finish(aiTicket, new Error('Synthetic transport fault'));
    first.enrichment.curate.aiAttempts.finish(aiTicket, 'failed');
    first.enrichment.curate.aiLimits.settleLimited({ role: 'stack', inputKey: 'f'.repeat(64),
      backendKey: 'e'.repeat(64), photoIds: ['timing-fixture'] });
...
function semanticSnapshot(installation) {
  ...
  return {
    ...
    curateAiAttempts: installation.enrichment.db.prepare('SELECT * FROM curate_ai_attempts ORDER BY role,input_key').all(),
    curateAiAges: installation.enrichment.db.prepare('SELECT * FROM curate_ai_attempt_age ORDER BY role,input_key').all(),
...
test('contract 24 retains charged attempts and snapshots before adding AI provider/photo limits', async () => {
  ...
  try {
    ...
    attempts.finish(attempts.start('stack', 'a'.repeat(64)), 'failed');
    const before = semanticSnapshot(installed);
    installed.enrichment.db.exec(`DROP TABLE curate_ai_attempt_age; DROP TABLE curate_ai_photo_charges;
      DROP TABLE curate_ai_backends; DROP TABLE curate_ai_skipped_inputs; PRAGMA user_version=19;`);
    closeInstallation(installed);
    const inventory = JSON.parse(readFileSync(config.persistentState.inventoryPath, 'utf8'));
    ...
    try {
      assert.equal(getUserVersion(prior), 19);
      assert.equal(prior.prepare("SELECT name FROM sqlite_master WHERE name='curate_ai_photo_charges'").get(), undefined);
      assert.equal(prior.prepare('SELECT attempts FROM curate_ai_attempts').get().attempts, 1);
    } finally { prior.close(); }
    closeInstallation(upgraded);
    const restoredConfig = fixtureConfig(join(workspace, 'restored'));
    restoreSnapshot(snapshotDir, restoredConfig);
    assert.equal(JSON.parse(readFileSync(restoredConfig.persistentState.inventoryPath, 'utf8')).upgrade.stateVersion, 24);
  } finally { rmSync(workspace, { recursive: true, force: true }); }
});

test('contract 23 snapshots the prior schema before adding the AI attempt ledger', async () => {
  const workspace = mkdtempSync(join(tmpdir(), 'pictaria-ai-attempt-upgrade-'));
  try {
    const root = join(workspace, 'source'); materializeLegacyInstallation(root);
    const config = fixtureConfig(root), installed = await openInstallation(config, 'initialize');
    const before = semanticSnapshot(installed);
    installed.enrichment.db.exec('DROP TABLE curate_ai_attempts; PRAGMA user_version=18;');
    closeInstallation(installed);
    const inventory = JSON.parse(readFileSync(config.persistentState.inventoryPath, 'utf8'));
    inventory.upgrade.stateVersion = 23;
    writeFileSync(config.persistentState.inventoryPath, JSON.stringify(inventory));
    ...
    const prior = new DatabaseSync(join(snapshotDir, 'enrichment.sqlite'), { readOnly: true });
    try {
      assert.equal(getUserVersion(prior), 18);
      assert.equal(prior.prepare("SELECT name FROM sqlite_master WHERE name='curate_ai_attempts'").get(), undefined);
    } finally { prior.close(); }
    closeInstallation(upgraded);
    const restoredConfig = fixtureConfig(join(workspace, 'restored'));
    restoreSnapshot(snapshotDir, restoredConfig);
    assert.equal(readFileSync(restoredConfig.settingsPath, 'utf8'), settings);
    assert.equal(JSON.parse(readFileSync(restoredConfig.persistentState.inventoryPath, 'utf8')).upgrade.stateVersion, 23);
  } finally { rmSync(workspace, { recursive: true, force: true }); }
});

for (const priorContract of [19, 20, 21]) test(`contract ${priorContract} upgrade snapshots before retaining settled check groupings`, async () => {
  const workspace = mkdtempSync(join(tmpdir(), 'pictaria-curate-retry-recovery-'));
  try {
    const root = join(workspace, 'source'); materializeLegacyInstallation(root);
    const config = fixtureConfig(root); const installed = await openInstallation(config, 'initialize');
    const before = semanticSnapshot(installed);
    const priorSchema = priorContract === 19 ? 16 : 17;
    installed.enrichment.db.exec(`DROP TRIGGER curate_rank_member_cleanup; DROP TABLE curate_rank_members;
      PRAGMA user_version = ${priorSchema};`);
    installed.enrichment.db.prepare('INSERT INTO curate_rank_evidence VALUES(?,?,?,?,?,?)')
      .run('legacy', 'connection', 'candidate-3', '{"rows":{},"coverage":{}}', 25, 1);
    if (priorContract === 20) installed.enrichment.db.exec('CREATE TABLE curate_rank_retries (scope_id TEXT);');
    closeInstallation(installed);
    const inventory = JSON.parse(readFileSync(config.persistentState.inventoryPath, 'utf8'));
...
test('contract 22 recovery point preserves legacy referee preferences before settings v8 migration', async () => {
  const workspace = mkdtempSync(join(tmpdir(), 'pictaria-curate-ai-recovery-'));
  try {
    const root = join(workspace, 'source'); materializeLegacyInstallation(root);
    const config = fixtureConfig(root);
    const installed = await openInstallation(config, 'initialize');
    ...
    const settings = JSON.parse(readFileSync(config.settingsPath, 'utf8'));
    settings.version = 7;
    settings.enrich.enabled = true;
    settings.curate = { burstGrouping: true, refereeEnabled: true, refereeModel: 'fixture-vision' };
    writeFileSync(config.settingsPath, JSON.stringify(settings));
    const inventory = JSON.parse(readFileSync(config.persistentState.inventoryPath, 'utf8'));
    inventory.upgrade.stateVersion = 22;
    writeFileSync(config.persistentState.inventoryPath, JSON.stringify(inventory));
    const upgraded = await openInstallation(config, 'verify');
    ...
    const current = readFileSync(config.settingsPath, 'utf8');
    closeInstallation(upgraded);
    const restarted = await openInstallation(fixtureConfig(root), 'verify');
    assert.equal(readFileSync(config.settingsPath, 'utf8'), current, 'no migration replay');
    closeInstallation(restarted);
    const restoredConfig = fixtureConfig(join(workspace, 'restored'));
    restoreSnapshot(snapshotDir, restoredConfig);
    assert.deepEqual(JSON.parse(readFileSync(restoredConfig.settingsPath, 'utf8')), settings);
    assert.equal(JSON.parse(readFileSync(restoredConfig.persistentState.inventoryPath, 'utf8')).upgrade.stateVersion, 22);
  } finally { rmSync(workspace, { recursive: true, force: true }); }
});
