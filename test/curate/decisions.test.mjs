...
test('schema-13 pending/dead jobs adopt local latest intent once; upgrade and restart do not alter decisions',async()=>{
 ...
 await f.restart();assert.equal(f.repo.db.prepare('PRAGMA user_version').get().user_version,17);assert.deepEqual(local(f,IDS[0]),tags);assert.equal(f.repo.deadSyncJobCount(),1);
