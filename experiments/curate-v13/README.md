...
| `src/enrich/refereeService.mjs` | Reuse bounded image fetch/source fallback and operational diagnostics. Preserve the owner-accepted keeper-quality baseline. Replace ten-image grouping, repaired ranks/subject groups and per-photo advice applicability in the new role integration. Remove unconditional yielding to any Enrich run through the shared scheduler. The unchanged released request contract is extracted in `src/enrich/referee-contract.mjs` and hash-pinned by the offline baseline evaluator. |
